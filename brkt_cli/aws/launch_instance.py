...
from encrypt_ami import wait_for_instance
...
def launch_instance(aws_svc, enc_svc_cls, image_id, metavisor_ami,
                    instance_name, subnet_id=None, security_group_ids=None,
    ...
    if 'brkt-avatar-freebsd' not in mv_image.name:
        raise BracketError(
            'Unsupported Bracket image for unencrypted guests: %s' %
            mv_image.name
        )
    legacy = False
    ...
    try:
        ...
        log.info('Launched unencrypted guest instance %s', instance.id)
        instance = wait_for_instance(aws_svc, instance.id)
        # TODO: Right now there is no way to determine when the Metavisor is READY
        # For now, just sleep for a pre-determined amount of time before proceeding
        #sleep(300)
        #bdm = instance.block_device_mapping
        # Create AMI from the running instance
        #stop_and_wait(aws_svc, instance.id)
        #name = get_name_from_image(guest_image)
        #description = get_description_from_image(guest_image)
        #ami = aws_svc.create_image(
        #    instance.id,
        #    name,
        #    description=description,
        ...
        #)
        #log.info('Registered AMI %s based on guest AMI %s', ami, guest_image.id)
        #wait_for_image(aws_svc, ami)
        #image = aws_svc.get_image(ami, retry=True)
        #name = NAME_METAVISOR_ROOT_SNAPSHOT
        #snap = image.block_device_mapping[image.root_device_name]
