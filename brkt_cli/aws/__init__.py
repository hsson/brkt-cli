...
def run_wrap_image(values, config):
    ...
    metavisor_ami = _encryptor_ami_from_values(values, config)
...
def run_encrypt(values, config, verbose=False):
    ...
    encryptor_ami = _encryptor_ami_from_values(values, config)
...
def _encryptor_ami_from_values(values, config):
    """ Return either the default Metavisor AMI ID or the one specified by
    --encryptor-ami or --metavisor-version.
    """
    brkt_env = brkt_cli.brkt_env_from_values(values, config)
    ...
    if not encryptor_ami:
        ...
        if not mv_version:
            ...
            log.debug(
                'Bracket service returned Metavisor version %s', mv_version)
        encryptor_ami = _get_encryptor_ami(values.region, mv_version)
    log.info('Using Metavisor %s', encryptor_ami)
    return encryptor_ami
